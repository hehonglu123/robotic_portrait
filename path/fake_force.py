...
start_length = 5  # Starting length
end_force = 2  # Ending force. Units: N
end_length = 5  # Ending length

### linear up #######
# read path from file
path = np.loadtxt("cartesian_path/strokes_out/0.csv", delimiter=",")
...
np.savetxt("force_path/strokes_out/0.csv", force_profile, delimiter=",")

### linear down #######
# read path from file
path = np.loadtxt("cartesian_path/strokes_out/1.csv", delimiter=",")
path_length = len(path)
# generate force profile
...
# save path with force
np.savetxt("force_path/strokes_out/1.csv", force_profile, delimiter=",")

### linear up and down, 2 segments #######
# read path from file
path = np.loadtxt("cartesian_path/strokes_out/2.csv", delimiter=",")
path_length = len(path)
# parameters
up_length = 10
down_length = 10
...
# save path with force
np.savetxt("force_path/strokes_out/2.csv", force_profile, delimiter=",") 
