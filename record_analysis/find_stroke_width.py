...
plt.scatter(data_pixel_w,data_force,label='data points')
plt.plot([min(data_pixel_w), max(data_pixel_w)], [A[0]*min(data_pixel_w)+A[1], A[0]*max(data_pixel_w)+A[1]], color='red',label='regression line')
plt.legend(fontsize=12)
plt.xlabel('Pixel width (pixel)',fontsize=12)
plt.ylabel('Force (N)',fontsize=12)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.title('Pixel width (pixel) vs force (N)',fontsize=16)
plt.show()